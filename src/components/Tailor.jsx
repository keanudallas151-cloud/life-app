...
export function TailorIntro({userName,onExplore,onTailor}){
  return(
    <div style={{minHeight:"100svh",paddingBottom:"env(safe-area-inset-bottom, 0px)",background:C.skin,display:"flex",flexDirection:"column",fontFamily:"Georgia,serif",overflowX:"hidden",touchAction:"pan-y"}}>
...
export function TailorQuestions({onComplete,onBack}){
  ...
  return(
    <div style={{minHeight:"100svh",paddingBottom:"env(safe-area-inset-bottom, 0px)",background:C.skin,display:"flex",flexDirection:"column",fontFamily:"Georgia,serif",overflowX:"hidden",touchAction:"pan-y"}}>
      ...
      <div style={{flex:1,padding:"36px 24px 24px",maxWidth:520,margin:"0 auto",width:"100%",boxSizing:"border-box",
        opacity:animDir==="out"?0:1,transform:animDir==="out"?"translateY(14px)":"translateY(0)",transition:"opacity 0.18s,transform 0.18s"}}>
            ...
            {q.opts.map(opt=>{
              ...
              return(
                <button key={opt} onClick={()=>toggleOpt(q.id,opt,q.multi)}
                  style={{width:"100%",boxSizing:"border-box",background:sel?"#6FBE77":C.white,border:`1.5px solid ${sel?"#6FBE77":C.border}`,borderRadius:18,padding:"15px 18px",fontSize:14,fontWeight:sel?700:400,color:sel?C.white:C.mid,cursor:"pointer",fontFamily:"Georgia,serif",transition:"all 0.15s",display:"flex",alignItems:"center",justifyContent:"flex-start",textAlign:"left",gap:10}}>
...
export function TailorResult({profile,userName,onContinue}){
